 ...
 *
 * 로그만 남긴다. prod 환경은 별도 외부 호출 어댑터가 본 빈을 대체 — application.yml 의
 * `security.notification.external=true` 시 본 빈이 비활성화되고 다른 구현이 활성화되도록 한다.
 */
@Component
...
class NoOpAlertNotification : AlertNotificationPort {

    override fun notify(alert: Alert) {
        log.info(
            "alert fired (no external notification configured): id={} severity={} rule={} group={}",
            alert.alertId, alert.severity, alert.ruleName, alert.groupKey,
        )
    }

    companion object {
        private val log = LoggerFactory.getLogger(NoOpAlertNotification::class.java)
    }
}
