 ...
 *
 * <p>tenantId 는 query 객체에 강제로 들어있으며, application layer 가 OpenSearch query 빌드 시
 * filter clause 로 다시 한 번 강제 주입한다 (사용자가 우회 불가).
 *
 * <p>검색 자체가 audit_entries 에 기록된다 (ISMS-P 2.9 통제).
 */
public interface SearchLogEventsUseCase {

  SearchResult search(SearchQuery query, OperatorContext operator);
}
