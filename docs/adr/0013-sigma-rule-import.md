...
SOC (Security Operations Center) 운영자는 *외부 보안 인텔리전스* 를 빠르게 수용해야 한다.
새로운 공격 패턴 (예: APT 그룹의 신규 TTP, 신규 멀웨어 IoC) 이 발견되면 우리 시스템에서도
*그날* 탐지 룰이 가동되어야 한다.
...
repo 로 관리한다. 운영자는 외부 룰 import → 우리 시스템 알람으로 변환 → 즉시 가동의
파이프라인이 필요하다.

매번 운영자가 *Sigma YAML 을 우리 룰 DSL 로 손번역* 하는 것은 비효율이고 오류 가능성도
크다. 자동 변환기가 필요하다.
...
2. `SigmaToAlertRuleMapper` — Sigma → 우리 `AlertRule` 변환 + `unsupported` (변환 불가 표현)
   목록 생성
...
본 단계의 룰 DSL 은 *단순 selection* 만 (e.g. `condition: selection`) 매핑한다. Sigma 의
...
이런 룰은 변환기가 *기본 골격만* 만들고 disabled 로 두며, 운영자가 별도 Flink CEP /
보강된 룰 DSL 로 수동 구현하도록 `unsupported` 에 명시한다.

### Severity 매핑

Sigma `level` → 우리 `Severity`:
...
자동 변환의 결과를 *그대로 가동* 하면 false-positive / 누락 가능. unsupported 가 비어있지
않은 룰은 `enabled=false` 로 영속 → 운영자가 검토 후 활성화. 안전 default.
...
- 우리 룰 DSL 과 *두 시스템* 이 공존 → 운영 일관성 깨짐

자동 변환 + 우리 룰 엔진 단일화가 더 깔끔.

### 외부 변환 도구 (sigmac CLI) 호출
탈락 — sigmac 가 ES query 로 변환해주지만, 우리 룰 DSL 의 *threshold + window + groupBy*
표현으로 가는 매핑은 우리가 직접 알아야 함. 결국 자체 변환기 필요.

## 결과

- Sigma 공개 룰 / 사내 인텔리전스 *그날 import* 가능 → SOC 대응 속도 향상
- 변환 한계 (unsupported) 가 명시 + 자동 disabled → false-positive / 누락 사고 차단
- 원본 YAML 보존 + audit → "어느 외부 룰이 우리 알람의 출처인가" 추적 가능
